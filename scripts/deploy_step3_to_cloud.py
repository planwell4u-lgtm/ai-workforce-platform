...
def deploy():
    tar_path = Path("d:/project/step3_update.tar.gz")
    ...
    scp_cmd = [
        ...
        str(tar_path),
        f"{HOST}:{REMOTE_DIR}/step3_update.tar.gz"
    ]
    ...
    remote_script = (
        f"cd {REMOTE_DIR} && "
        "tar -xzf step3_update.tar.gz && "
        "rm -f step3_update.tar.gz && "
    ...
    if tar_path.exists():
        tar_path.unlink()
    print("Step 3 deployment completed successfully!")
